        ...
        <div class="w-full md:w-8/12 lg:w-6/12 flex flex-col items-center md:flex-row">
            ...
            <div class="md:w-8/12 lg:w-6/12 px-5 flex flex-col items-center md:justify-center md:items-start py-10 md:py-10 ">
                ...
                <div class="flex items-center gap-4">
                    <p class="text-gray-700 text-1xl font-bold mt-2">Nombre: {{ $user->name }}</p>
                    @auth
                    ...
                    @if($user->id !== auth()->user()->id)
                        ...
                        @else    
                            <form action="{{ route('users.unfollow', $user) }}" method="POST">
                                @method('DELETE')
                                @csrf
                                <input type="submit" class="bg-red-600 text-white uppercase rounded-lg px-3 py-1 text-xs font-bold cursor-pointer mt-5" value="dejar de seguir">
                            </form>
                        @endif
                    @endif
                @endauth
